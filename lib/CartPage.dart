  ...
  Future<void> packAllItemsInTheirContainer() async {
    if(widget.cardList.length>0){
      for(int i = 0;i<widget.cardList.length;i++){
        ...
        String marketPrice = await GetBookDetails().getBookMarketPrice(bookAddress);
        ...
        String imagePath = await GetBookDetails().getBookCoverPathAddress(bookAddress);
      ...
      AppConstants().defaultSnackBarForApp(context, "Opps! you don't have any item in your cart");
    }
  }
    ...
    if(_database!=null && uid!=null)
      {
        ...
        result = await data.once().then((DataSnapshot dataSnapshot) {
          ...
          values.forEach((key, value) {
            ...
            temp.add(CardForCartPage(key,refreshListOfItems),);
            setState(() {
              widget.cardList = temp;
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: MediaQuery.of(context).size.height/MediaQuery.of(context).size.height/50,),
                          ElevatedButton(
                    ...
                    Expanded(
                      child: CachedNetworkImage(
                        imageUrl: 'https://media.giphy.com/media/QxdbjzetcgDImLeeSO/giphy.gif',
                        placeholder: (context, url) => Container(child: CircularProgressIndicator(),height: 50,width: 50,),
