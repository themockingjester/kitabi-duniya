...
class _DeliveryPageState extends State<DeliveryPage> {
  Future<bool> doWeHaveRequiredQuantityOfBooks(
      String bookAddress, EncapsulatedContainerForNewBook book) async {
    int availablePieces =
        int.parse(await GetBookDetails().getBookAvailablePieces(bookAddress));
              ...
              if (RegExp(AppConstants().phoneNumberValidateRegexString)
                ...
                if (widget.type != 'sell') {
                  ...
                  for (int j = 0; j < widget.books.length; j++) {
                    ...
                    String bookAddress =
                        '$bookName@$authors@$publisher@$status@$edition';
                    if (await doWeHaveRequiredQuantityOfBooks(
                            bookAddress, widget.books[j]) ==
                        false) {
                      return;
                    }
                  }
                  ...
                  if (widget.type == "sell") {
                    ...
                    if (result == 'done') {
                      await DeliveryDetails()
                          .setSellerDeliveryDetailsForPending(
                    ...
                    if (result == 'done') {
                      ...
                      String bookAddress =
                          '$bookName@$authors@$publisher@$status@$edition';
                      await SetBookDetails().setAvailablePieces(
                          context, bookAddress, widget.books[j].bookQuantity);
