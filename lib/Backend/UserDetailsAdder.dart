...
class UserDetailsAdder {
  ...
  Future<void> addUserInterestedTags(
    ...
    if (userId != null) {
      await _database.child(pathForUserDetails).child(userId).update({
        ...
        AppConstants().defaultSnackBarForApp(
            context, "Thanks for telling us your interest!");
      }).onError((error, stackTrace) {
        AppConstants().defaultSnackBarForApp(
            context, 'Something went wrong with database');
      });
    } else {
      AppConstants().defaultSnackBarForApp(
          context, 'Something went wrong best suggestion is restart this app!');
    }
  ...
  Future<void> addNewUserDetails(String name, String email, String dob,
