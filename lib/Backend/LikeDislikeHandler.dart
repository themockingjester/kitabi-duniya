  ...
  Future<void> dislikeThisBook(String bookAddress, BuildContext context) async {
    ...
    if (_database != null && uid != null) {
      if (isAlreadyDisliked == null) {
        ...
        var wheatherUserLikingExist =
            await checkUserLikingForTheBook(bookAddress);
        if (wheatherUserLikingExist == true) {
          int currentTotalLikes =
              int.parse(await GetBookDetails().getBookTotalLikes(bookAddress));
          currentTotalLikes--;
        ...
        _database.child(path).child('dislikes').child(bookAddress).set({
          ...
          AppConstants().defaultSnackBarForApp(
              context, "We will try to improve next time!");
        }).onError((error, stackTrace) {
          AppConstants().defaultSnackBarForApp(context,
              "We couldn't setup communication with our database! Try after sometime");
        });

        int currentTotalDislikes =
            int.parse(await GetBookDetails().getBookTotalDisLikes(bookAddress));
  ...
  Future<void> likeThisBook(String bookAddress, BuildContext context) async {
    ...
    if (_database != null && uid != null) {
      if (isAlreadyLiked == null) {
        ...
        var wheatherUserDislikingExist =
            await checkUserDislikingForTheBook(bookAddress);
        if (wheatherUserDislikingExist == true) {
          int currentTotalDislikes = int.parse(
              await GetBookDetails().getBookTotalDisLikes(bookAddress));
          currentTotalDislikes--;
        ...
        await _database.child(path).child('likes').child(bookAddress).set({
          ...
          AppConstants()
              .defaultSnackBarForApp(context, "Thanks for your like!");
        }).onError((error, stackTrace) {
          AppConstants().defaultSnackBarForApp(context,
              "We couldn't setup communication with our database! Try after sometime");
        });
