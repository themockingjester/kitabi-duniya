...
class SearchEngine {
    ...
    if (_database != null && uid != null) {
      ...
      if (uid != null) {
        ...
        for (int i = 0; i < list.length; i++) {
          String smallString =
              await SearchEngine().covertAllToSmallerCase(list[i]);
          List<String> splittedWords =
              await SearchEngine().convertIntoList(smallString);
          for (int j = 0; j < splittedWords.length; j++) {
            var checkingExistence =
                await wheatherInputIsAKeywordTag(splittedWords[j]);
            if (checkingExistence == false) {
              if (isWordCapableOfBecomingKeywordTag(splittedWords[j])) {
                await _database
                    .child(commonPathForKeywordTags)
                    .child(splittedWords[j])
                    .set({bookIdentificationName: "true"});
              }
            } else if (checkingExistence == true) {
              await _database
                  .child(commonPathForKeywordTags)
                  .child(splittedWords[j])
                  .update({bookIdentificationName: "true"});
            }
          }
        }
        ...
        for (int j = 0; j < tags.length; j++) {
          ...
          for (int k = 0; k < splittedPartsOfTags.length; k++) {
            ...
            var checkingExistence =
                await wheatherInputIsAKeywordTag(splittedPartsOfTags[k]);
            if (checkingExistence == false) {
              if (isWordCapableOfBecomingKeywordTag(splittedPartsOfTags[k])) {
                await _database
                    .child(commonPathForKeywordTags)
                    .child(splittedPartsOfTags[k])
                    .set({bookIdentificationName: "true"});
              }
            } else if (checkingExistence == true) {
              await _database
  ...
  Future<void> fetchRequiredBooks(String input, var functionToAddBooksInList,
      String cardType, Map addedBooks) async {
    String uid = _auth.currentUser!.uid;
        ...
        if (cardType == "big") {
          ...
          values.forEach((key, value) async {
            if (!addedBooks.containsKey(key)) {
              addedBooks[key] = true;
              String imagePath =
                  await GetBookDetails().getBookCoverPathAddress(key);
