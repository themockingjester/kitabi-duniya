...
class SetBookDetails{
  ...
  late String result;
  Future<String> addBookWithVerification(BuildContext context,EncapsulatedContainerForNewBook bookDetails) async {
    ...
    coverPath = 'books/incoming/${bookIdentificationName}/${coverPath.hashCode}$extension';
    if(uid!=null){

      await _database.child(commonPathForIncomingBooks).child(uid).child(bookIdentificationName).set({
        'name': bookName,
        'authors': authors,
      ...
      AppConstants().defaultSnackBarForApp(context, "you don't have permission!");
      return 'failed';
    }
  }

  Future<String> buyThisBook(BuildContext context,EncapsulatedContainerForNewBook bookDetails) async {
    String uid=_auth.currentUser!.uid;
    String bookName = bookDetails.bookName;
    String authors = bookDetails.authorName;
    String publisherName = bookDetails.bookPublisher;
    ...
    if(uid!=null){
      ...
      await _database.child(commonPathForOutgoingBooks).child(uid).child(bookIdentificationName).set({
        'name': bookName,
        'authors': authors,
        'quantity':'$quantity',
        'publisher':publisherName,
  ...
  Future<void> setAvailablePieces(BuildContext context,String bookAddress,String toBeSold) async {
    String uid=_auth.currentUser!.uid;
    String currentPieces = await GetBookDetails().getBookAvailablePieces(bookAddress);
    int leftOnes = int.parse(currentPieces)-int.parse(toBeSold);
    if(uid!=null){

      await _database.child(commonPathForBooks).child(bookAddress).update({
        'available pieces':'$leftOnes'
      }).whenComplete(() {
        // AppConstants().defaultSnackBarForApp(context, "We have received your order!");
        // result = 'done';
      }).onError((error, stackTrace) {
  ...
  Future<void> addBookWithOutVerification(BuildContext context,EncapsulatedContainerForNewBook bookDetails) async {
    String bookName = bookDetails.bookName;
    String authors = bookDetails.authorName;
    String publisherName = bookDetails.bookPublisher;
    String bookStatus = bookDetails.bookStatus;
    ...
    coverPath = 'books/main/${bookIdentificationName}/${coverPath.hashCode}$extension';

    if(await GetUserDetails().checkUserSuperiority()=='exist' && uid!=null && _database!=null){
      String bookExistence = await GetBookDetails().checkIfBookExist(bookIdentificationName);
      int bookQuantity = int.parse(quantity);
      if(bookExistence=='not exist'){

        await _database.child(commonPathForBooks).child(bookIdentificationName).set({
          'name': bookName,
          'authors': authors,
        ...
        bookQuantity=int.parse(await GetBookDetails().getBookAvailablePieces(bookIdentificationName))+bookQuantity;
        await _database.child(commonPathForBooks).child(bookIdentificationName).update(
          {
            'available pieces':'$bookQuantity',
            'market price':marketPrice,
            'our price':ourPrice,
      ...
      AppConstants().defaultSnackBarForApp(context, "You don't have the permission!!");
    }
  }
  Future<void> addNewBook(BuildContext context,bool DirectTrigger,EncapsulatedContainerForNewBook bookDetails) async {
    String bookName = bookDetails.bookName;
    String authors = bookDetails.authorName;
    String publisherName = bookDetails.bookPublisher;
    String bookStatus = bookDetails.bookStatus;
    ...
    if(DirectTrigger){
      if(await GetUserDetails().checkUserSuperiority()=='exist'){
        String bookIdentificationName = '$bookName@$authors@$publisherName@$bookStatus@$bookEdition';
        String res = await Storage().uploadImageDirectly(bookIdentificationName, imagePath, extension);
        if(res=='done'){
          // image has been uploaded!
          String? coverPath=Storage().coverPathForDirectUpload(bookIdentificationName, imagePath, extension);
          if(coverPath!=null){
      ...
      String bookIdentificationName = '$bookName@$authors@$publisherName@$bookStatus@$bookEdition';
      String res = await Storage().uploadIncomingImage(bookIdentificationName, imagePath, extension);
      if(res=='done')
        {
          String? coverPath = Storage().coverPathForIncomingImageUpload(bookIdentificationName, imagePath, extension);
          if(coverPath!=null){
            String res = await Storage().uploadIncomingImage(bookIdentificationName, imagePath, extension);
            if(res=='done'){
